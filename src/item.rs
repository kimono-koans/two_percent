...
impl Drop for ItemPool {
    fn drop(&mut self) {
        let _reserved_items = std::mem::take(&mut *self.reserved_items.lock());
        let _pool = std::mem::take(&mut *self.pool.lock());
    }
}
...
impl ItemPool {
    ...
    pub fn reserved(&self) -> ItemPoolGuard<'_, Weak<dyn SkimItem>> {
